			...
			<Listbox value={selected} onChange={(e) => {setSelected(e); handleUpdateParams(e)}}>
				<div className="relative w-fit z-10">
					<Listbox.Button className="custom-filter__btn">
						<span className="truncate block text-black">{selected.title}</span>
						<Image
						...
						<Listbox.Options className="custom-filter__options">
							{options.map((option) => (
								<Listbox.Option
									...
									value={option}
								>
									{({ selected }) => <span className={`block truncate ${selected ? "font-medium": "front-normal"}`}>{option.title}</span>}
								</Listbox.Option>
							))}
						</Listbox.Options>
					</Transition>
